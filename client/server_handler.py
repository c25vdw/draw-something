...
class ServerHandler(socket.socket, threading.Thread):
    ...
    def __init__(self, client_ip, server_ip, event_hub):
        ...
        self.player_id = -1   
        self.event_hub = event_hub # used to upload sketches and drawing out on local.(in main)
        self.canDraw = None
    ...
    def connect(self):
        self.sendto(GameServer.COMMAND_CLIENT_CONNECT.encode('utf-8'), self.server_ip)
        return
    ...
    def update_pygame_from_json(self, data_dumped):
        ...
        self.score = data_json["score"] # obj {"player_1": int, "player_2": int}
